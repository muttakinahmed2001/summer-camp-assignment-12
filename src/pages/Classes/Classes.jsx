...
const Classes = () => {
  ...
  const { data: classesByStatus = [] } = useQuery(
    ["approveClasses"],
    async () => {
      const response = await axiosSecure.get(
        "/classesByStatus?status=Approved"
      );
  ...
  const handleSelectButton = (languageClass) => {
    ...
    if (user && user.email) {
      ...
      fetch(
        "https://assignment-12-server-one-sepia.vercel.app/selectedClasses",
        {
          method: "POST",
          headers: {
          ...
          <div
            ...
            <div className="card-body items-center text-center">
              ...
              <div className="card-actions">
                <button
                  disabled={
                    isAdmin || isInstructor || languageClass.AvailableSeat === 0
                      ? true
                      : false
                  }
                  onClick={() => handleSelectButton(languageClass)}
